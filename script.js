...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Update the number of visible slides based on window size
  function updateVisibleSlides() {
      visibleSlides = window.innerWidth >= 768 ? 3 : 1;
      updateSliderPosition();
  }
  ...
  function prevSlide() {
      slideIndex -= visibleSlides;
      if (slideIndex < 0) {
          slideIndex = totalSlides - visibleSlides; // Loop to the end
      }
      updateSliderPosition();
  }
  ...
  // Testimonial Slider (for duplicate and cyclic effect)
  const slides2 = document.querySelector(".slides2");
  const slideTestimonials = document.querySelectorAll(".slide2");
  const totalSlides2 = slideTestimonials.length;
  let currentIndex2 = 0;
  ...
  // Show testimonial slide based on index
  function showTestimonialSlide(index) {
      slides2.style.transition = "transform 1s ease-in-out";
      slides2.style.transform = `translateX(-${index * 100}%)`;
  }

  // Show next testimonial slide
  function showNextTestimonialSlide() {
      currentIndex2++;
      if (currentIndex2 === totalSlides2) {
          // Move to the position of the duplicated first testimonial instantly
  ...
  // Show previous testimonial slide
  function showPreviousTestimonialSlide() {
      currentIndex2--;
      if (currentIndex2 < 0) {
          // Move to the position of the last testimonial instantly
  ...
  // Attach event listeners to buttons
  document.querySelector(".next-button2").addEventListener("click", showNextTestimonialSlide);
  document.querySelector(".prev-button2").addEventListener("click", showPreviousTestimonialSlide);
});
...
//     .then((result) => {
//       let element = document.getElementById("blog-wrapper");
//       let list1 = document.createDocumentFragment();
//       result.posts.forEach(posts => {
//         console.log(posts);
//         let divion = document.createElement("div")
//         divion.className = "blog-box"
//         let divison2 = document.createElement("div")
//         divison2.className = "blog-image-content"
//         divion.appendChild(divison2)
//         let header = document.createElement("h1")
//         let text = document.createElement("p")
//         header.innerHTML = `${posts.title}`
//         text.innerHTML = `${posts.text}`
//         divion.appendChild(header)
//         divion.appendChild(text)
//         list1.appendChild(divion)
